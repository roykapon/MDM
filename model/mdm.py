...
class MDM(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        """
        assert (y is not None) == (self.cond_mode != "no_cond"), "must specify y if and only if the model is class-conditional"
        ...
        if "text" in self.cond_mode:
            ...
            # added by roy
            # embed_text = self.embed_text(self.mask_cond(enc_text, force_mask=force_mask))

            # DIFFUSION_STEPS_NUM = 1000
            # relative_diffusion_step = timesteps.reshape(timesteps.shape[0], 1) / DIFFUSION_STEPS_NUM

            # we want that initially the text is more significant (the relative_diffusion_step is initially 0)
            # as the we get to deeper diffusion steps, we increase the effect of the already generated motion

            # print(f"emb.shape: {emb.shape}")
            # print(f"timesteps.shape: {timesteps.shape}")

            # emb = emb * relative_diffusion_step + embed_text * (1 - relative_diffusion_step)

        if "action" in self.cond_mode:
            action_emb = self.embed_action(y["action"])
            emb += self.mask_cond(action_emb, force_mask=force_mask) * relative_diffusion_step
